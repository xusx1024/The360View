package com.shengxingg.opengleslib.glutils;
...
public class OGLUtils {
  ...
  private static int getProgram(String vertexStr, String fragmentStr) {
  ...
  private static String getShaderSource(Context context, String sourceName) {
    StringBuffer shaderSource = new StringBuffer();
    try {
      BufferedReader br =
          new BufferedReader(new InputStreamReader(context.getAssets().open(sourceName)));
      String tempStr = null;
  ...
  protected static int bindTexture(Context context, int drawable) {
    ...
    return bindTexture(BitmapFactory.decodeResource(context.getResources(), drawable, options));
  }
  ...
  // 初始化返回纹理的ID
  public static int initTexture(Context context, int drawable) {
    int[] textures = new int[1];
    glGenTextures(1, textures, 0);
